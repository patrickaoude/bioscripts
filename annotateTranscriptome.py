...
HMMSEARCHPATT = re.compile("b'\s+(?P<full_e>[\de\-\.]+)\s+(?P<full_score>[\de\-\.]+)\s+(?P<full_bias>[\de\-\.]+)\s+(?P<best_domain_e>[\de\-\.]+)\s+(?P<best_domain_score>[\de\-\.]+)\s+(?P<best_domain_bias>[\de\-\.]+)\s+(?P<exp>[\de\-\.]+)\s+(?P<N>[\de\-\.]+)\s+(?P<model>[A-Za-z0-9-_]+)\s+(?P<description>[A-Za-z0-9\s_\-\(\)]+)")
HMMSEARCHPATTKEY = ["full_e", "full_score", "full_bias", "best_domain_e", "best_domain_score", "best_domain_bias", "exp", "N", "model", "description"]
TMPDIR = "tmp"

@click.command()
@click.option("-i", "--infile", required=True, type=str, help="Specify the fasta file containing sequences to annotate.")
@click.option("-o", "--outfile", required=True, type=str, help="Specify the name of the tsv output file containing all annotations.")
@click.option("-h", "--hmmerbin", type=str, default="/lab/solexa_reddien/Patrick/tools/hmmer-3.3.2/bin", help="Specify the path to hmmer's bin directory.")
@click.option("-m", "--model", type=str, default="/lab/solexa_reddien/Patrick/tools/annotations/pfamIndexFiles/Pfam-A.hmm", help="Specify the path to the hmm model to use for annotations.")
@click.option("-c", "--cores", type=int, default=mp.cpu_count(), help="Specify the number of CPUs or cores for parallel processing.")
def annotateSequences(infile, outfile, hmmerbin, model, cores):
    ...
    with open(outfile, "w") as f:
        ...
        pool = mp.Pool(cores)
        results = []
        for result in tqdm(pool.imap_unordered(processSequence, sequences), total=numSeqs):
...
def findDomains(record):
    ...
    hmmscan = subprocess.Popen(f"{scanPath} --notextw {hmmfile} {orfPath}", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    try:
    ...
    if len(orfs):
        ...
        if result is None:
            ...
            if allNames:
                ...
                names = None
            if allDescriptions:
                descriptions = [f"{k} ({v})" for k,v in Counter(allDescriptions).items()]
        ...
        noORF = True
    row = [sequence.id, names, descriptions, noORF, noFeatures, str(sequence.seq), translatedSequence]
    return(row)
