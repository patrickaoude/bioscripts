...
@click.command()
@click.option("-c", "--contigs", required=True, type=str, help="Specify a comma-separated list of dd_smed_v6 contig to fetch GO terms for.")
@click.option("-o", "--outfile", required=True, type=str, help="Specify the output path for the results (results are in tab-separated format or .tsv).")
def queryContigs(contigs, outfile):
    contigs = contigs.split(",")
    output = [queryContig(c) if "dd_Smed_v6_" in c else [{"contig": c, "name": "", "description": "incorrect contig format (need dd_Smed_v6_)"}] for c in contigs] # only query correctly formatted IDs (regex would be better)
...
def queryContig(contig):
    ...
    for row in query.rows():
        term = {
            ...
            "description": row["domainHits.proteinDomain.goAnnotation.ontologyTerm.description"]
        }
        terms.append(term)
    return(terms)
